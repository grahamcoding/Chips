import React, {Component}  from 'react';
  ...
  class Math extends Component {
    ...
    constructor(props) {
      super(props);
      this.state = {
        margin: '0.00', //States defined for the math used to calculate profit margins.
        dipmargin: '0.00'
      }
    }
    ...
    getNum(val) {   
      ...
      val = val.toFixed(2) //Shortens and rounds values to 2 decimal places.
      return val;
   }
    ...
    render() {
      ...
      const { fiveshelf } = this.props // 5 Shelf Toggle pulled from parent's state to change layout based on shelf size selection.

      const margin = this.state.margin //Hold input values for margins on products
      const dipmargin = this.state.dipmargin //Hold input values for margins on dip shelf products

      //Math for 3 Shelf Displays. Hidden by fiveshelf toggle.
      ...
      //Math for 5 Shelf Displays. Hidden by fiveshelf toggle.
